...
case 'addtodo':
	if ( $atd_option['list_view'] == '0' || current_user_can($atd_option['add_capability']) ) {
		...
		$deadline = attribute_escape($_POST['atd_deadline']);
		atd_insert($todotext, $priority, $assign, $deadline);
...
case 'updatetd':
	if ( $atd_option['list_view'] == '0' || current_user_can($atd_option['edit_capability']) ) {
		...
		$assign = attribute_escape($_POST['atd_assign']);
		$deadline = attribute_escape($_POST['atd_deadline']);
		atd_update($id, $priority, $todotext, $assign, $deadline);
...
function atd_insert($todotext, $priority, $assign, $deadline) {
...
function atd_update($id, $priority, $todotext, $assign, $deadline) {
   	global $wpdb, $userdata, $atd_option;
   	get_currentuserinfo();
...
function atd_install () {
	...
	if (current_user_can('install_plugins')) {
   	...
   	if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
		...
		add_option( 'atd_db_version', $atd_db_version );
		}

	$installed_ver = get_option( 'atd_db_version' );

	//if( $installed_ver != $atd_db_version ) {
    	...
    	update_option( 'atd_db_version', $atd_db_version );
		//}
	}
...
function atd_todo_subpanel() {
	...
	<div class="wrap">
 		...
 		<form name="edittd" action="tools.php?page=cleverness-to-do-list" method="post">
			...
			<?php endif; ?>
			<?php if ($atd_option['show_deadline'] == '1') : ?>
				<th scope="row"><label for="atd_deadline"><?php _e('Deadline', 'cleverness-to-do-list') ?></label></th>
				<td><input type="text" name="atd_deadline" id="atd_deadline" value="<?php echo wp_specialchars($todo->deadline, 1); ?>" /></td>
			</tr>
			<?php endif; ?>
	   		<tr>
	...
	<div class="wrap">
		...
		<tr>
			...
			<?php if ( $atd_option['show_deadline'] == '1' ) : ?><th><?php _e('Deadline', 'cleverness-to-do-list'); ?></th><?php endif; ?>
		...
		elseif ( $atd_option['list_view'] == '1' && $atd_option['show_only_assigned'] == '0' )
   		...
   		if ($results) {
			...
			if ( $atd_option['show_deadline'] == '1' ) $colspan += 1;
	...
	<div class="wrap">
    	...
    	<form name="addtd" id="addtd" action="tools.php?page=cleverness-to-do-list" method="post">
			...
			<tr>
		  		<th scope="row"><label for="atd_assign"><?php _e('Assign To', 'cleverness-to-do-list') ?></label></th>
		  		<td>
			   		<select name='atd_assign' id='atd_assign' class=''>
						<option value='-1'>None</option>
						<?php
						$contributors = getUsersWithRole('contributor');
		...
		<?php// wp_dropdown_users('show_option_none=None&name=atd_assign'); ?>
			</tr>
			<?php endif; ?>
			<?php if ($atd_option['show_deadline'] == '1') : ?>
				<th scope="row"><label for="atd_deadline"><?php _e('Deadline', 'cleverness-to-do-list') ?></label></th>
				<td><input type="text" name="atd_deadline" id="atd_deadline" value="" /></td>
			</tr>
			<?php endif; ?>
			<tr>
        		<th scope="row" valign="top"><label for="atd_description"><?php _e('To-Do', 'cleverness-to-do-list') ?></label></th>
